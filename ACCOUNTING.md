...
| Quantity | Value |
|---|---|
| Decidable theorems (chained receipts) | **2,912** |
| Signed statement receipts | **32** |
| Coins per receipt | **2** |
| Coins on the ledger (theorems × 2) | **5,824** |
| Coins on signed receipts (× 2) | **64** |
| Released versions (git tags) | **919** |
| Tracked, content-addressed files | **539** |

Latest release: **v9.7.1**. The fair-exchange unit is **2 coins = 2 bits** (110 − 108 = 2 = −χ genus-2) per receipt. One 64-bit harmony coin is minted per fused `src` `report()` module — see the [state dashboard](/dashboard) for the harmonic root.

## Bounty — denominated in bits

The bounty for each accepted contribution is **2 bits (2 coins)** — the same fair-exchange unit, earned by the deed (a gate-passing, receipted contribution) and owed by commercial use. Total bounty accounted on the ledger so far: **5,824 bits**. This is an accounting bounty in bits/coins — **integrity, not a cash prize**. Heroes and traitors by deeds, not claims.
...
Page content-address: `824cae1a-ff9c-8d36-8541-a0679d627c1b`. Integrity, not truth.
