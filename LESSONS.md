# Lessons — what the tree has learned

Derived on every run from 280 sources: **128 corrections** the deposit recorded about itself,
in the comments of the files where each one happened. Nobody wrote this list; it is counted.

The taxonomy is a choice — six classes, because six are what the record shows — and it is stated so a
reader can disagree with the classes rather than with the arithmetic.

| corrections | class | why it costs something |
| ---: | --- | --- |
| 45 | a constant typed instead of derived | the value and its meaning drift apart, and the copy is the one nobody updates |
| 32 | a check that could not go red | it reports health it never measured, and deleting it would change nothing |
| 23 | the instrument was wrong, not the code | a finding is evidence about the finder until the finder has been run against a known case |
| 12 | domain narrower than the defect | the claim reads over the whole tree and the scan covered part of it |
| 9 | the check read its own explanation | a gate that scans the tree scans the prose describing what it scans for |
| 7 | a flattering number survived | a result that clears you is checked less than one that accuses you |

## Where they were learned

A file near the top has been corrected often. That is a record of attention, not of poor quality —
the untouched files are the ones nobody has checked this hard.

- `scripts/gates-fire.ts` — 11
- `src/proof/instruments.lean` — 6
- `scripts/contradictions.ts` — 5
- `scripts/paper.ts` — 5
- `scripts/uses.ts` — 5
- `scripts/orphan-gate.ts` — 5
- `scripts/control-probe.ts` — 4
- `scripts/pages.ts` — 4

_Generated by `scripts/lessons.ts`. It excludes itself: every pattern above appears in it, because it_
_explains them, and a check that reads the whole tree reads its own explanation._
