...
Derived on every run from 296 sources: **145 corrections** the deposit recorded about itself,
...
| ---: | --- | --- |
| 51 | a constant typed instead of derived | the value and its meaning drift apart, and the copy is the one nobody updates |
| 34 | a check that could not go red | it reports health it never measured, and deleting it would change nothing |
| 28 | the instrument was wrong, not the code | a finding is evidence about the finder until the finder has been run against a known case |
...
| 10 | a flattering number survived | a result that clears you is checked less than one that accuses you |
