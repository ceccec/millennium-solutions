...
Of 534 machine-checked declarations, **501** restate work that already has an author and
**33** are about this deposit's own construction. Each source file declares which it is, in its own
...
| | theorems |
|---|---|
| attributed to named earlier work | **501** |
| unclassified — no search performed, status unknown | **33** |
| claimed as novel | **0** |

**Zero claims is not full attribution.** Stated alone, "this deposit claims no novelty" reads as a concession
that everything here already has an author. It is not that. **33** theorems have had no prior-art
...
33 theorems on the strength of their own self-description, with nobody having looked. Asserting
that nothing earlier exists because no one went to check is the same defect as asserting a proof because no
one went to read it.

## Restated from named earlier work — 26 sources, 501 theorems
...
<tr><td><code>generated.lean</code></td><td>14</td><td>elementary number theory — the unit group of ℤ/9 and the doubling orbit</td><td>the structure underneath is standard and is credited. The doubling orbit</td></tr>
...
<tr><td><code>quantum.lean</code></td><td>9</td><td>canonical forms and order-invariant commitments</td><td>sorting a multiset into a canonical order BEFORE folding it is standard practice, not a</td></tr>
...
- **canonical forms and order-invariant commitments** — 9 theorems, in `quantum.lean`
...
## This deposit's own construction — 4 sources, 33 theorems, none claimed
...
Partition seal `aa0ae6dd-ddc7-8482-a559-5ddcf4113f2f` · recompute with `node scripts/priorart.ts` · the kernel re-decides
