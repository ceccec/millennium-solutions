...
Of 1,216 machine-checked declarations, **1,199** restate work that already has an author and
**17** are about this deposit's own construction. Each source file declares which it is, in its own
...
| | theorems |
|---|---|
| attributed to named earlier work | **1,199** |
| unclassified — no search performed, status unknown | **17** |
| claimed as novel | **0** |

**Zero claims is not full attribution.** Stated alone, "this deposit claims no novelty" reads as a concession
that everything here already has an author. It is not that. **17** theorems have had no prior-art
...
17 theorems on the strength of their own self-description, with nobody having looked. Asserting
that nothing earlier exists because no one went to check is the same defect as asserting a proof because no
one went to read it.

## Restated from named earlier work — 61 sources, 1,199 theorems
...
<tr><td><code>instruments.lean</code></td><td>29</td><td>elementary order theory, list processing and string matching</td><td>NO NOVELTY IS CLAIMED AND NONE IS DENIED. The three rules are elementary — a strict order</td></tr>
...
- **elementary order theory, list processing and string matching** — 29 theorems, in `instruments.lean`
...
## This deposit's own construction — 2 sources, 17 theorems, none claimed
...
Partition seal `aa585925-9885-8619-b959-d77d31ffb19c` · recompute with `node scripts/priorart.ts` · the kernel re-decides
