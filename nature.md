...
const ENTRIES = [
  ...
  { kind: 'fact', where: 'An honest second', what: 'The second reads back exactly from the metre and from the caesium period, for every whole number of seconds — the same measure for everyone, everywhere.', keys: ['lean_light_the_second_returns_from_the_metre_and_the_period_for_every_s'] },
