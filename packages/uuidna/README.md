...
The npm label is **frozen at `0.1.1`** (a held label, by directive). `package.json#version` is the single
source of truth — the MCP server and CLI read it; no version is hardcoded anywhere. The true "latest" is the
**content-address**, not the label: identical content keeps its address, and a real delta moves it, so
integrity — not a rising number — tells you whether anything changed.

Two odometers, kept distinct so they don't read as a bug:

- **This package** — the frozen `0.1.1` npm label; releases are distinguished by content-address and by
  signed git tags (immutable provenance), not by bumping the label.
- **The deposit it is extracted from** (`ceccec.psg.bg/millennium-solutions`) — its own single-digit
  odometer (`major.minor.patch`, each `0..9`, rolling at 9; **LTS** minors are the Fibonacci numbers
  `{1, 2, 3, 5, 8}`). Its `7.x` number is the *site's* state, not this package's.
