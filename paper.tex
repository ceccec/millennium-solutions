...
(\texttt{npm run latex-gate}). There are 541 of them across 30 files and
6 wings; 533 close by exhaustion over a stated finite domain and the
...
\begin{theorem}[\texttt{the\_witness\_is\_about\_the\_state\_not\_the\_machine}]
\[ \left|\mathrm{ghzXSupport}\right| \cdot 2 = \left|\{0,\dots,7\}\right| \land 2^{3} = 8 \]
\end{theorem}
\noindent\emph{Proof.} \texttt{by decide} --- exhausting 8 cases. \hfill$\square$
...
\begin{theorem}[\texttt{an\_encoding\_changes\_width\_not\_the\_count\_of\_operations}]
\[ \mathrm{hexbitChars} < \mathrm{hexChars} \land \mathrm{hexbitMs} > \mathrm{hexMs} \land \frac{1048576}{20} = 52428 \land \frac{1048576}{20} = 52428 \]
\end{theorem}
\noindent\emph{Proof.} \texttt{by decide} --- by evaluation; no domain is walked. \hfill$\square$
