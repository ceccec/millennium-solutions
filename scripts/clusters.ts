...
const reached = new Map(fileList.map((f) => [f, 0]))
{
  ...
  for (let s = 0; s < SHUFFLES; s++) {
    ...
    const t = topLifts(l)
    for (const f of fileList) if (t.get(f)! >= observedTop.get(f)! - 1e-9) reached.set(f, reached.get(f)! + 1)
  }
}
...
function neighbours(c: Cluster) {
  ...
  return { centre, within: +within.toFixed(2), between: +between.toFixed(2), top: ranked.slice(0, 3), chanceShare: chanceShare(c.file), shuffles: SHUFFLES }
}
...
function deposition(c: Cluster) {
  ...
  const html = [
    `<p><strong>${esc(c.title)}</strong> — ${c.sealed.length} machine-checked theorems of the ℤ/9 vortex deposit, one Lean namespace, `
      + `wing <em>${esc(c.wing)}</em>. ${decided} close by exhaustion over a finite domain (<code>by decide</code>)${proved ? `, ${proved} are proved for every value on the standard axioms` : ''}`
      + `${c.declarations.length ? `; ${c.declarations.length} <code>rfl</code> declaration(s) are listed and not sealed, by design` : ''}. Every one is sealed live in the append-only ledger.</p>`,
    `<p><strong>How these theorems are bound.</strong> "Entangled" is used in this deposit's honest sense: correlation by derivation, `
      + `not quantum entanglement, and no information travels through it.</p>`,
    `<p><strong>1 · Shared definitions.</strong> ${s.binding.length} definition(s) are used by two or more theorems, binding ${s.edges} pair(s); `
      + `the theorems fall into ${s.strands.length} strand(s)${s.strands.length ? `, the largest of ${s.strands[0].length}` : ''}.`
      + (s.binding.length ? ` Most binding: ${s.binding.slice(0, 6).map(([d, ns]) => `<code>${esc(d)}</code> (${ns.length})`).join(', ')}.` : '')
      + (s.loose.length ? ` ${s.loose.length} theorem(s) name no definition of the file and stand on their own statement.` : '') + `</p>`,
    `<p><strong>2 · Imports.</strong> Built on: ${importsOf(c.file).map((f) => `<code>${f}</code>`).join(', ') || 'no other cluster'}. `
      + `Built on it: ${dependentsOf(c.file).map((f) => `<code>${f}</code>`).join(', ') || 'no other cluster'}. Checking this cluster needs ${files.length} file(s): <code>${files.join('</code>, <code>')}</code>.</p>`,
    `<p><strong>3 · The joint address.</strong> The ${c.sealed.length} sealed receipts fold to <code>${j.root}</code>. `
      + `Order-invariant: ${j.orderInvariant ? 'yes — the same root in reverse order' : 'NO'}. Altering one receipt changes it: ${j.bindsEveryReceipt ? 'yes, computed on this run' : 'NO'}. `
    ...
    `<p><strong>4 · The address lattice.</strong> Each theorem sits at the seven ray values its page plots from its receipt, taken mod 9: a point of (ℤ/9)⁷. `
      + `This cluster's centre is (${n.centre.join(', ')}). Its nearest clusters, ranked by lift — pairs observed among each theorem's seven nearest points, over the pairs expected by chance from the two clusters' sizes: `
      + `${n.top.map((x) => `<code>${x.file}</code> (${x.pairs} observed, ${x.expected} expected, lift ${x.lift})`).join(', ') || 'none with three or more pairs'}. A lift near 1 is what chance gives. Mean distance between two of its theorems: ${n.within}; `
      + `from one of its theorems to a theorem of another cluster: ${n.between}. A receipt is a hash, so nearness on this lattice is a property of the addresses, not of the mathematics; `
      + `the mathematical binding is (1) and (2). Chance, measured: over ${n.shuffles} reshufflings of which cluster each point belongs to (seeded, so every rebuild `
      + `gives the same numbers), a best lift at least this high arose in ${(n.chanceShare * 100).toFixed(1)}% of them — `
      + `${n.chanceShare >= 0.05 ? 'consistent with chance' : 'above chance by this measure, though with 33 clusters about two are expected there by chance alone'}.</p>`,
    `<p><strong>Prior art.</strong> ${kind === 'named' ? esc(note) : 'Declared ' + esc(kind || 'unclassified') + '.'}`
      + `${credits.size ? ` Credited per theorem: ${[...credits].map(([t, cr]) => `<code>${esc(t)}</code> — ${esc(cr)}`).join('; ')}.` : ''}`
      // A PERMANENT RECORD CARRIES NO TALLY THAT WILL CHANGE. The per-theorem searches live in a ledger that is searched
      // again as sources allow (two of four were rate-limiting on the first run), so the record points at it instead.
      ...
      + `in the living ledger <a href="${REPO}/blob/main/src/proof/novelty.json">src/proof/novelty.json</a>. No novelty is claimed here.</p>`,
    ...
    `<p><strong>Recompute.</strong> <code>lake env lean ${files.join(' ')}</code>, or clone <a href="${REPO}">${REPO}</a> and run <code>npm run lean</code>. `
      + `This record was generated by scripts/clusters.ts from the source and the ledger.</p>`,
    `<p><strong>Funding.</strong> ${esc(FUNDING.statement)}</p>`,
  ].join('')
  return {
    upload_type: 'publication', publication_type: 'preprint',
    title: `${c.title} — ${c.sealed.length} machine-checked theorems of the ℤ/9 vortex deposit and how they are bound`,
    creators: zen.creators, description: html, license: zen.license, access_right: zen.access_right ?? 'open', language: 'eng',
    keywords: ['Lean 4', 'machine-checked proof', 'formal verification', 'Z/9', 'content-addressing', 'theorem cluster', c.wing, c.file.replace('.lean', '')],
    communities: zen.communities ?? [],
    related_identifiers: [
      { identifier: CONCEPT_DOI, relation: 'isPartOf' },
      { identifier: `${REPO}/blob/main/src/proof/${c.file}`, relation: 'isSupplementTo' },
      ...importsOf(c.file).map((f) => ({ identifier: `${REPO}/blob/main/src/proof/${f}`, relation: 'references' })),
      ...c.sealed.slice(0, 60).map((t) => ({ identifier: `${SITE}/theorem/${keyOf(t)}`, relation: 'hasPart' })),
    ],
    notes: `cluster ${c.file} · wing ${c.wing} · ${c.sealed.length} sealed · joint address ${j.root} · lattice centre (${n.centre.join(',')}) · `
      + `neighbours ${n.top.map((x) => x.file).join(', ') || '—'} · concept DOI ${CONCEPT_DOI} · generated by scripts/clusters.ts — prepared, not uploaded`,
    files,
    cluster: { file: c.file, wing: c.wing, sealed: c.sealed.length, decided, proved, declarations: c.declarations.length, joint: j, strands: s.strands.length,
      binding: s.binding.slice(0, 12).map(([d, ns]) => ({ def: d, theorems: ns.length })), imports: importsOf(c.file), dependents: dependentsOf(c.file), lattice: n },
  }
}
...
for (const { c, d } of out) {
  ...
  console.log(`  ${c.file.padEnd(18)} ${String(k.sealed).padStart(3)} sealed · ${String(k.strands).padStart(2)} strand(s) · joint ${k.joint.root.slice(0, 13)}… · `
    + `binds every receipt ${k.joint.bindsEveryReceipt ? '✓' : '✗'} · lattice neighbours by lift ${k.lattice.top.map((x) => `${x.file.replace('.lean', '')}(${x.pairs}/${x.expected}=${x.lift})`).join(' ') || '—'} · within ${k.lattice.within} / between ${k.lattice.between} · chance ${k.lattice.chanceShare}`)
}
{
