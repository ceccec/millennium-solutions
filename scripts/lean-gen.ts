...
// THE LEAN LIST, interpolated. These templates carried the text `units()` — a TypeScript call written
// inside a Lean string with no `${}` around it — so regenerating this file emitted `units()` into the
// proof and the kernel rejected it. The committed artefact has the literal list, which is how the drift
// stayed invisible: scripts/lean-gen.ts runs in no chain, so nobody re-ran it to find out.
const U = '[' + units.join(',') + ']'
// AS A RENDERED LIST, like U above. The template below contained the literal text `axis()` — a TypeScript
// call written into Lean, which does not parse. `axis` became a function in src/api at some point and this
// template was never updated, so `lean-gen --emit` has been emitting a file the kernel refuses. Nothing
// caught it because nothing RUNS this generator: it is recorded in UNRUN_BY_DESIGN as a generator, and it
// only surfaced when a negative control was written for it. An unrun gate can rot; so can an unrun
// generator, and this one had.
const A = '[' + axis().join(',') + ']'
...
// ATTRIBUTION IS CARRIED FORWARD, NEVER REGENERATED AS `unclassified`. This header emitted a fixed
// `-- prior_art: unclassified`, so re-running --emit ERASED the prior-art block a search had written into
// generated.lean — the doubling orbit as the unit group U(9), named with its terms and date. The generator
// replaced a credited attribution with a claim of no known prior art, which is the one direction this
// deposit must never move in.
//
// This is the SECOND generator with this defect. scripts/imagine.ts had it and was fixed hours earlier, and
// I did not think to check its sibling — the fix went to the instance rather than to the shape, which is the
// mistake this whole session keeps finding. It surfaced here only because running lean-gen's new negative
// control regenerated the file and the attribution gate caught the loss.
// The whole CONTIGUOUS block, in order — not a list of keys. Keying it lost `prior_art_pool` (absent from
// my list) while keeping its continuation lines, producing a header that read as an orphaned fragment. An
// attribution is prose written by a person: carry the region, not the fields someone remembered.
const priorPrev = existsSync('src/proof/generated.lean') ? readFileSync('src/proof/generated.lean', 'utf8').split('\n') : []
const firstArt = priorPrev.findIndex((l) => /^-- prior_art/.test(l))
let lastArt = firstArt
if (firstArt >= 0) {
  for (let i = firstArt; i < priorPrev.length; i++) {
    if (/^-- prior_art/.test(priorPrev[i]) || /^--\s{2,}\S/.test(priorPrev[i])) lastArt = i
    else if (priorPrev[i].startsWith('-- ') || priorPrev[i].trim() === '') break
  }
}
const carriedArt = firstArt >= 0 ? priorPrev.slice(firstArt, lastArt + 1) : []
const artBlock = carriedArt.length && carriedArt[0] !== '-- prior_art: unclassified'
  ? carriedArt.join('\n') + '\n' : '-- prior_art: unclassified\n'

let body = `set_option maxRecDepth 100000\n\n-- title: Generated at scale\n-- wing: the machine\n${artBlock}-- Generated by scripts/lean-gen.ts — do not edit by hand; re-run the generator.\n-- Each theorem below quantifies over a whole ledger family. Every one is compiled, audited for axioms, and\n-- checked to compute what the ledger's own tests compute at every parameter of its family.\n\nnamespace Generated\n\n`
