...
const receipt: Record<string, unknown> = { uuid, message, agent, role }
if (invites.length) receipt.invites = invites
writeFileSync(dir + '/' + uuid + '.json', JSON.stringify(receipt, null, 2) + '\n')
console.log('✓ receipt (payload): ' + agent + ' as ' + role + (invites.length ? ' · invites ' + invites.length + ' theorem(s)' : '') + ' → src/receipts/' + uuid + '.json')
