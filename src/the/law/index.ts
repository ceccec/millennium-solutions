...
export function report(): string {
  const LAW = [
    ...
    'the sequence — measure → gate → receipt (observer and role) → append (the chain) → recompute (every build): follow it in order, or nothing computes.',
  ...
  o += '\n  law root ' + root.slice(0, 13) + '… — one order-independent fold of the license, the sequence, the exchange, and the bound.'
  return o
}
